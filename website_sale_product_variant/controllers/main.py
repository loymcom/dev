...
class WebsiteSaleProductVariant(WebsiteSale):
    ...
    def _shop_lookup_products(self, attrib_set, options, post, search, website):
        ...
        # THE CODE BELOW IS COPIED FROM website_sale and  # MODIFIED #  1 place ("model").

        product_count, details, fuzzy_search_term = website._search_with_fuzzy("products_only", search,
                                                                               limit=None,
                                                                               order=self._get_search_order(post),
                                                                               options=options)
        search_result = details[0].get('results', request.env[model]).with_context(bin_size=True)  # MODIFIED #
